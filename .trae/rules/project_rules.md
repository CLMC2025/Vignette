...
- **类型声明限制**：禁止使用内联对象字面量作为类型声明（例如 `as { foo?: string }`），必须改为具名 `interface`/`type`。
- **JSON 解析**：`JSON.parse()` 后必须转成显式结构类型；禁止使用 `any`/`unknown`/`Record<string, unknown>` 作为中间类型。
...
- **Builder 语法**：`@Builder` 内只能写 UI 组件语法，不允许 `return` 早退或插入普通语句块；条件渲染用 `if (cond) { ... }`。
